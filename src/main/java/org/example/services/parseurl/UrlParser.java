            ...
            try {
                ...
                for (Element element : tagA) {
                    ...
                    if (isUrlCorrect(lowerCaseElementUrl)) {
                        ...
                        subTask.setLemmatizator(lemmatizator);
                        subTask.fork();
                        tasks.add(subTask);
                    }
                }
    ...
    public void insertData(Connection connection) throws IOException, SQLException, InterruptedException {
        ...
        if (responseCode == 200) {
            Map<String, Lemma> lemmaMap = lemmaService.createAndInsertLemmaOnDuplicateUpdateAndGetMap(site,
                    lemmatizator.getLemmaSet(bodyText + " " + titleText));
            Map<String, Float> titleLemmasCount = lemmatizator.countLemmasOnField(titleText);
            Map<String, Float> bodyLemmasCount = lemmatizator.countLemmasOnField(bodyText);
            Map<String, Float> lemmasAndRank = lemmatizator.calculateLemmasRank(lemmaMap, titleLemmasCount, bodyLemmasCount);
